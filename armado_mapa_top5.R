...
tabla_graf_top10 <- centroides_aglomerados %>% 
  mutate(AGLOMERADO = as.factor(AGLOMERADO)) %>%
  left_join(select(tasa_des, AGLOMERADO, tasa_desocupacion), 
            by = "AGLOMERADO") %>% 
  slice_max(tasa_desocupacion, n = 10) %>% 
  mutate(
...
tabla_graf_top_menos10 <- centroides_aglomerados %>% 
  mutate(AGLOMERADO = as.factor(AGLOMERADO)) %>%
  left_join(select(tasa_des, AGLOMERADO, tasa_desocupacion), 
            by = "AGLOMERADO") %>% 
  slice_min(tasa_desocupacion, n = 10) %>% 
  mutate(
    col_y = rank(-tasa_desocupacion) %>% 
      data_rescale(to=c(-15, -65)),
    sigmoid_end = 22,
...
ggplot(tabla_graf_top10) +
  ...
  labs(
    title="Tasa de desocupación por Aglomerados urbanos - Top 10 aglomerados",
    subtitle = "",
...
ggsave("salidas/ranking_tasa_desocupacion_top10.png",width = 25, height = 10, dpi = 300)
dev.off()


ggplot(tabla_graf_top_menos10) +
  geom_sf(data=mapa_arg, size = .3, fill = "transparent", color = "gray", expand = TRUE) +
  geom_point(aes(x=lon, y=lat, color=tasa_desocupacion)) +
  geom_sigmoid(
    aes(x=lon, y=lat, xend=sigmoid_end, yend=col_y, group=nombre_aglomerado, color=tasa_desocupacion)
...
ggsave("salidas/ranking_tasa_desocupacion_top_menos10.png",width = 25, height = 10, dpi = 300)
dev.off()
